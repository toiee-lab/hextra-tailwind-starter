...
キーワードを抽出する際は、以下の優先順位に従って選択してください:

### 🔴 最優先（必ず含める）
1. **ページタイトル**: Front Matterの`title`フィールドの主要な単語
2. **主要見出し**: H2、H3レベルの見出しに含まれるキーワード
3. **ドメイン固有用語**: 業界特有の専門用語、製品名、サービス名
4. **固有名詞**: 人名、地名、企業名、ブランド名

例: `Hugo`, `Hextra`, `Tailwind CSS`, `Claude Code`, `API`, `セットアップ`

### 🟡 高優先（重要なキーワード）
5. **技術用語**: プログラミング言語、フレームワーク、ツール名
6. **アクションワード**: ユーザーが検索しそうな動詞（設定、インストール、デプロイなど）
7. **問題解決ワード**: トラブルシューティングに関連する用語（エラー、解決、修正など）
8. **頻出概念**: ページ内で複数回言及される重要な概念

例: `インストール`, `設定`, `カスタマイズ`, `ビルド`, `デプロイ`, `トラブルシューティング`

### 🟢 中優先（補助的なキーワード）
9. **関連技術**: 関連するツールやライブラリ（直接言及されていなくても文脈から推測可能）
10. **カテゴリ用語**: コンテンツのカテゴリやセクションを表す用語
11. **同義語**: 主要キーワードの言い換えや類義語
12. **バリエーション**: 日本語の場合、漢字/ひらがな/カタカナのバリエーション

例: `ドキュメント/文書`, `開始/スタート`, `構成/コンフィグ`

### ⚪ 低優先（必要に応じて含める）
13. **補足説明**: コンテンツの補足的な概念
14. **よくある誤字**: ユーザーが間違えそうなスペル
15. **略語展開**: 略語の正式名称（既に略語がある場合）

例: `CSS` → `Cascading Style Sheets`, `API` → `Application Programming Interface`

### ❌ 除外すべきキーワード
- 一般的すぎる単語: `もの`, `こと`, `について`, `です`, `ます`
- 冠詞・助詞: `の`, `を`, `に`, `は`, `が`
- 短すぎる単語: 1-2文字の意味の薄い単語
- ページに無関係な単語: コンテンツと関連性の低い用語
